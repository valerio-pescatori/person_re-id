...
{
    public static ZScoreOutput StartAlgo(List<decimal> decimalInput, int lag, double threshold, double influence)
    {
        // coverting input decimal to double
        List<double> input = new List<double>(decimalInput.Count);
        foreach(decimal dec in decimalInput)
            input.Add((double) dec);
